...
async function runTest() {
    ...
    if (errA || errB || errC)
        throw new Error('Failed to create admin users: ' + (errA?.message || errB?.message || errC?.message));
    ...
    if (profileErr)
        throw new Error('Failed to insert public users: ' + profileErr.message);
    ...
    const clientA = (0, supabase_js_1.createClient)(SUPABASE_URL, SUPABASE_ANON_KEY, { auth: { persistSession: false } });
    const clientB = (0, supabase_js_1.createClient)(SUPABASE_URL, SUPABASE_ANON_KEY, { auth: { persistSession: false } });
    const clientC = (0, supabase_js_1.createClient)(SUPABASE_URL, SUPABASE_ANON_KEY, { auth: { persistSession: false } });
    ...
    if (errChatsA)
        console.error('errChatsA:', errChatsA);
    if (errChatsA || !chatsA || chatsA.length === 0)
        throw new Error('RLS Failure: User A cannot see their own chat.');
    // User C should NOT see the chat
    const { data: chatsC, error: errChatsC } = await clientC.from('chats').select('*').eq('id', chat.id);
    if (errChatsC)
        throw new Error(errChatsC.message);
    if (chatsC && chatsC.length > 0)
        throw new Error('RLS Failure: User C CAN see a private chat they are not in.');
    ...
    if (!insertErrC)
        throw new Error('RLS Failure: User C was able to insert a message into a private chat they are not in.');
    ...
    if (!insertErrA)
        throw new Error('RLS Failure: User A was able to insert directly. Client inserts should be blocked by default unless explicitly allowed.');
...
//# sourceMappingURL=test-db-rls.js.map