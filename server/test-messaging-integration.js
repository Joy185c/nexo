...
async function runTests() {
    ...
    try {
        ...
        if (!searchRes || searchRes.length === 0 || searchRes[0].username !== bUsername)
        ...
        if (!chatListA.some((c) => c.id === chatId))
        ...
        if (msgRes.content !== 'Hello User B!')
        ...
        const bChat = chatListB.find((c) => c.id === chatId);
        if (!bChat || bChat.last_message.content !== 'Hello User B!')
            throw new Error('Recipient chat list failed');
        ...
        if (msgsB.length === 0 || msgsB[0].content !== 'Hello User B!')
...
runTests();
//# sourceMappingURL=test-messaging-integration.js.map