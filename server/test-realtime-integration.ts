...
async function runTests() {
  ...
  try {
    ...
    if (readA.read_count !== 0) throw new Error('Sender marked their own message as read');
    ...
    if (readB.read_count !== 1) throw new Error(`Recipient marked incorrect number of messages as read: ${readB.read_count}`);
